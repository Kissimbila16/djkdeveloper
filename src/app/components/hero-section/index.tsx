...
export default function HeroSection() {
  ...
  // Type guard to check if canvas is HTMLCanvasElement
  const isCanvas = (canvas: HTMLCanvasElement | null): canvas is HTMLCanvasElement => {
    return canvas !== null;
  };
  ...
  useEffect(() => {
    ...
    if (!isCanvas(canvas)) return; // Early exit with type guard

    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    const resizeCanvas = () => {
      if (isCanvas(canvas)) { //check again inside resizeCanvas
    ...
    class Particle {
      ...
      constructor() {
        if (isCanvas(canvas)) { //check inside constructor
          this.x = Math.random() * canvas.width;
          this.y = Math.random() * canvas.height;
          this.size = Math.random() * 3 + 1;
          this.speedX = Math.random() * 1 - 0.5;
          this.speedY = Math.random() * 1 - 0.5;
          this.color = `rgba(${Math.floor(Math.random() * 100 + 155)}, ${Math.floor(Math.random() * 100 + 155)}, 255, ${
            Math.random() * 0.5 + 0.1
          })`;
        } else {
          // Handle the case where canvas is null (e.g., set default values)
          this.x = 0;
          this.y = 0;
          this.size = 1;
          this.speedX = 0;
      ...
      update() {
        if (isCanvas(canvas)) { //check inside update
          this.x += this.speedX;
          this.y += this.speedY;
    ...
    <div className="container mx-auto px-6 z-20">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.6 }}>
          <p className="text-xl md:text-2xl text-gray-300 mb-8">Construindo o futuro da web com JavaScript</p>
        </motion.div>
